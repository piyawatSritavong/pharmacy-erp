...
7. 🐛 **อุบัติเหตุระหว่างทดสอบ + กันซ้ำแล้ว:** การรัน `next build` ขณะ dev server รันอยู่ทับโฟลเดอร์ `.next` เดียวกัน ทำให้ dev server เสิร์ฟหน้าไม่มี CSS (test พฤติกรรมยังผ่านทั้งที่หน้าเปล่า!) → กัน 2 ชั้น: (a) `npm run build:check` ใช้ `NEXT_DIST_DIR=.next-build` แยกโฟลเดอร์ ไม่แตะ dev server, (b) เพิ่ม test "styles: pages render with the compiled stylesheet applied" ใน manual.spec.ts ตรวจ computed style จริง — หน้าไม่มี CSS จะ fail ทันที (suite ตอนนี้ 16 tests)
